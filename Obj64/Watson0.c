...
static INTEGER Watson0_syspos;
static BOOLEAN Watson0_sysImported, Watson0_first, Watson0_showObj;


static void Watson0_Append (CHAR *d, INTEGER d__len, CHAR *s, INTEGER s__len);
static void Watson0_AppendCh (CHAR *d, INTEGER d__len, CHAR ch);
static void Watson0_Ch (CHAR ch);
static void Watson0_Commands (Files_File F, CHAR *modName, INTEGER modName__len);
static void Watson0_Control (CHAR *name, INTEGER name__len, CHAR *cmd, INTEGER cmd__len, CHAR *par, INTEGER par__len);
static void Watson0_Find (OfrontOPT_Object root, CHAR *name, INTEGER name__len, OfrontOPT_Object *obj);
static void Watson0_FindPointer (OfrontOPT_Object root, CHAR *name, INTEGER name__len, OfrontOPT_Object *obj);
...
static void Watson0_Identifier (CHAR *name, INTEGER name__len);
static void Watson0_Indent (INTEGER i);
static void Watson0_Ln (void);
static void Watson0_MakeText (Texts_Text T);
static void Watson0_Num (LONGINT i);
static OfrontOPT_Object Watson0_ReadSym (CHAR *mod, INTEGER mod__len, CHAR *fileName, INTEGER fileName__len);
static void Watson0_SetOptions (void);
export void Watson0_ShowCmd (CHAR *file, INTEGER file__len, Texts_Text T);
export void Watson0_ShowDef (CHAR *mod, INTEGER mod__len, Texts_Text T);
export void Watson0_ShowObj (CHAR *mod, INTEGER mod__len, CHAR *objName, INTEGER objName__len, Texts_Text T);
static void Watson0_Str (CHAR *s, INTEGER s__len);
...
static void Watson0_WriteModule (CHAR *mod, INTEGER mod__len, OfrontOPT_Object root);
...
{
	__DUP(s, s__len);
	Texts_WriteString(&Watson0_W, Texts_Writer__typ, s, s__len);
	__DEL(s);
}

static void Watson0_Ch (CHAR ch)
{
	Texts_Write(&Watson0_W, Texts_Writer__typ, ch);
}

static void Watson0_Num (LONGINT i)
{
	Texts_WriteLongInt(&Watson0_W, Texts_Writer__typ, i, 0);
}
...
static void Watson0_Ln (void)
{
	Texts_WriteLn(&Watson0_W, Texts_Writer__typ);
}
...
static void Watson0_Identifier (CHAR *name, INTEGER name__len)
{
	...
	Watson0_Control(name, name__len, (CHAR*)"", 1, (CHAR*)"", 1);
	__DEL(name);
}
...
static void Watson0_WriteConstValue (OfrontOPT_Object obj)
{
	...
	switch (obj->typ->form) {
		case 4: case 5: case 6: 
			Texts_WriteLongInt(&Watson0_W, Texts_Writer__typ, y, 0);
			break;
		case 7: 
			Texts_WriteReal(&Watson0_W, Texts_Writer__typ, obj->conval->realval, 15);
			break;
		case 8: 
			Texts_WriteLongReal(&Watson0_W, Texts_Writer__typ, obj->conval->realval, 20);
			break;
		case 2: 
			if (y == 0) {
				Watson0_Str((CHAR*)"FALSE", 6);
			} else {
				Watson0_Str((CHAR*)"TRUE", 5);
			}
			break;
		case 3: 
			if (y >= 32 && y <= 126) {
...
static void Watson0_WriteForm (OfrontOPT_Struct typ, INTEGER i)
{
	if (typ->form == 15 && typ->comp == 6) {
		Watson0_Str((CHAR*)"RECORD", 7);
		Watson0_WriteBase(typ);
		if (Watson0_showAdr) {
			Watson0_Str((CHAR*)" [size = ", 10);
		...
		Watson0_Str((CHAR*)"END", 4);
	} else if (typ->form == 15 && typ->comp == 2) {
		Watson0_Str((CHAR*)"ARRAY ", 7);
		Watson0_Num(typ->n);
		Watson0_Str((CHAR*)" OF ", 5);
		Watson0_WriteType(typ->BaseTyp, i);
	} else if (typ->form == 15 && typ->comp == 5) {
		Watson0_Str((CHAR*)"ARRAY OF ", 10);
		Watson0_WriteType(typ->BaseTyp, i);
	} else if (typ->form == 15 && typ->comp == 4) {
		Watson0_Str((CHAR*)"ARRAY * OF ", 12);
		Watson0_WriteType(typ->BaseTyp, i);
	} else if (typ->form == 13) {
		Watson0_Str((CHAR*)"POINTER TO ", 12);
	} else if (typ->form == 14) {
		Watson0_Str((CHAR*)"PROCEDURE", 10);
...
static void Watson0_WriteType (OfrontOPT_Struct typ, INTEGER i)
{
	...
	if (typ->strobj != NIL && typ->strobj->name[0] != 0x00) {
		...
		if (makeLink) {
			...
			Watson0_Control((CHAR*)"", 1, cmd, 32, par, 32);
			Texts_SetColor(&Watson0_W, Texts_Writer__typ, 15);
		}
	} else {
...
static void Watson0_WriteModule (CHAR *mod, INTEGER mod__len, OfrontOPT_Object root)
{
	...
	Watson0_Str((CHAR*)"DEFINITION ", 12);
	Watson0_Str(mod, mod__len);
	if (Watson0_showAdr) {
		Watson0_Str((CHAR*)" [", 3);
		Texts_WriteLongHex(&Watson0_W, Texts_Writer__typ, root->adr);
		Watson0_Ch(']');
	}
	Watson0_Ch(';');
	Watson0_Ln();
...
static void Watson0_MakeText (Texts_Text T)
{
	Texts_Append(T, Watson0_W.buf);
	if (Watson0_sysImported && !Watson0_showObj) {
		Texts_Insert(T, Watson0_syspos, Watson0_W.buf);
	}
}
...
static void FindP__8 (CHAR *name, INTEGER name__len, OfrontOPT_Object root, OfrontOPT_Object *obj);
...
static OfrontOPT_Object Watson0_ReadSym (CHAR *mod, INTEGER mod__len, CHAR *fileName, INTEGER fileName__len)
{
	...
	if (f != NIL || __STRCMP(mod, "SYSTEM") == 0) {
		if (__STRCMP(mod, "SYSTEM") == 0) {
			Console_String((CHAR*)"definition of SYSTEM not available", 35);
			Watson0_Ln();
			return NIL;
		} else {
		...
		if (ch == 0xbb) {
			...
			Texts_Open(T, (CHAR*)"", 1);
			Texts_OpenReader(&RR, Texts_Reader__typ, T, 0);
			...
			Console_String((CHAR*)"-- not a symbol file", 21);
			Console_Ln();
		}
	} else {
		Console_String((CHAR*)"--  symbol file not found", 26);
		Console_Ln();
	}
	return NIL;
}
...
static void Watson0_Commands (Files_File F, CHAR *modName, INTEGER modName__len)
{
	...
	Files_ReadChar(&R, Files_Rider__typ, &ch);
	if (ch != 0xbb) {
		Console_String((CHAR*)"not a object file", 18);
		return;
	}
	Files_ReadChar(&R, Files_Rider__typ, &ch);
	...
	Files_ReadChar(&R, Files_Rider__typ, &ch);
	if (ch != 0x82) {
		Console_String((CHAR*)"corrunpted object file", 23);
		return;
	}
	while (nofentry > 0) {
	...
	while (nofcmds > 0) {
		...
		Watson0_Ch('.');
		Watson0_Str(name, 32);
		Watson0_Control(name, 32, (CHAR*)"", 1, (CHAR*)"", 1);
		Watson0_Ln();
		nofcmds -= 1;
	}
	Watson0_Str((CHAR*)"END ", 5);
...
void Watson0_ShowDef (CHAR *mod, INTEGER mod__len, Texts_Text T)
{
	CHAR file[64];
	OfrontOPT_Object root = NIL;
	Watson0_SetOptions();
	__COPY(mod, file, 64);
	Console_String((CHAR*)"reading ", 9);
...
/*----------------------------------------------------------------------------*/
void Watson0_ShowObj (CHAR *mod, INTEGER mod__len, CHAR *objName, INTEGER objName__len, Texts_Text T)
{
	...
	if (root != NIL) {
		...
		if (obj != NIL) {
			...
			Console_String((CHAR*)"\' not found", 12);
			Console_Ln();
		}
	}
}
...
/*----------------------------------------------------------------------------*/
void Watson0_ShowCmd (CHAR *file, INTEGER file__len, Texts_Text T)
{
	Files_File F = NIL;
	CHAR fileName[64];
	Watson0_SetOptions();
...
/* BEGIN */
	...
	Texts_OpenWriter(&Watson0_W, Texts_Writer__typ);
	__ENDMOD;
}
